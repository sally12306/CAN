...
class TrainConfig:
    ...
    offline_iterations: int = int(5_0000)  # Number of offline updates
    online_iterations: int = int(5_0000)  # Number of online updates
    checkpoints_path: Optional[str] = "/hd_2t/fuhao2024/Yzc/CaI_ql_model"  # Save path
...
@torch.no_grad()
def eval_actor(
        env: gym.Env, actor: nn.Module, list1: List, list2: List, list3: List, list4: List, device: str, n_episodes: int, seed: int
):
    global cnt
    parser = argparse.ArgumentParser('Parse configuration file')
    parser.add_argument('--env_config', type=str, default='/hd_2t/fuhao2024/Yzc/NEW_cav copy/CrowdNav/crowd_nav/configs/env.config')
...
def train(config: TrainConfig):
    ...
    file_path = "/hd_2t/fuhao2024/Yzc/NEW_cav copy/CrowdNav/Multi-Policy_Dataset/Multi-Policy_Dataset.hdf5"
    ...
    for t in range(int(config.offline_iterations) + int(config.online_iterations)):
        ...
        if t >= config.offline_iterations:
            ...
            #     online_log["train/success_rate"] = success / (success + collision + run_time)
            #     # episode_return = 0
            #     # episode_step = 0
            #     # goal_achieved = False
        ...
        if (t + 1) % config.eval_freq == 0:
            ...
            # 取本次评估的统计指标
            # success_rate = list1[-1]
            # collision_rate = list2[-1]
            # avg_nav_time = list3[-1]
            # avg_reward = list4[-1]
            ...
            # eval_log["eval/avg_nav_time"] = avg_nav_time
            # eval_log["eval/avg_reward"] = avg_reward
