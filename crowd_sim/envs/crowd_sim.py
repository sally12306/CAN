...
class CrowdSim(gym.Env):
    ...
    def step(self, action, update=True):
        ...
        cur_dist = norm(np.array([self.robot.px, self.robot.py]) - np.array(self.robot.get_goal_position()))
        end_position = np.array(self.robot.compute_position(action, self.time_step))
        end_dist = norm(end_position - np.array(self.robot.get_goal_position()))

        reaching_goal = end_dist < self.robot.radius
        # print('dmin',dmin)
    ...
    def render(self, mode='human', output_file=None):
        ...
        if mode == 'human':
            ...
            # 保存图片到文件并退出
            self.idx +=1
            plt.savefig(f'./image/iql-rnd/trajectory_{self.idx}.png', bbox_inches='tight')
            plt.close()
        elif mode == 'video':
            fig, ax = plt.subplots(figsize=(7, 7))
            ax.tick_params(labelsize=16)
